...
- Map surfaces use procedurally textured PBR materials from `client/src/game/materials.ts`, keyed by `classifySurface`; deterministic noise helpers live in `client/src/game/procgen.ts`. Materials and box geometries are cached module-level and intentionally shared across maps. The `graphicsQuality` setting is persisted in localStorage.
